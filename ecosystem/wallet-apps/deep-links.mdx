...
This page provides an overview of approaches for issuing invoices with links for TON wallets.
...
USDT decimals are 6. This means 1 USDT unit equal 1000000 in amount value.
...
To generate a transaction link, the service must obtain the account address, logical time (lt), and transaction hash by calling the [getTransactions](https://toncenter.com/api/v2/#/transactions/get_transactions_getTransactions_get) method. Using these values, [tonviewer.com](https://tonviewer.com), [tonscan.org](https://tonscan.org), and [explorer.toncoin.org](https://explorer.toncoin.org/) render the transaction page according to the following formats:

Tonviewer and Tonscan also support external-in message hashes instead of transaction hashes for explorer links. This is useful when generating an external message and needing an instant transaction link.
